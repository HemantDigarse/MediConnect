...
const STATUS_CONFIG = {
  ...
  COMPLETED: { label: 'Completed', badge: 'badge-blue',   icon: CheckCircle },
}

const PAY_CONFIG = {
  PENDING:  { label: 'Payment Pending', cls: 'text-amber-700 bg-amber-50' },
  PAID:     { label: 'Paid',           cls: 'text-emerald-700 bg-emerald-50' },
  REFUNDED: { label: 'Refunded',       cls: 'text-[#737686] bg-[#F2F3FF]' },
}
  ...
  return (
    <div className="card flex flex-col gap-4">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs text-[#737686] font-medium uppercase tracking-wide mb-1">{specialty}</p>
          <h4 className="font-bold text-[#131B2E]">{doctorName ? `Dr. ${doctorName}` : patientName}</h4>
        </div>
      ...
      {/* Date/Time */}
      <div className="flex items-center gap-4 p-3 bg-[#F2F3FF] rounded-btn">
      ...
      <div className="flex items-center justify-between">
        <span className={`text-xs font-semibold px-2.5 py-1 rounded-badge ${payCfg.cls}`}>{payCfg.label}</span>
        <span className="font-bold text-[#131B2E]">₹{consultationFee}</span>
      </div>
      ...
      <div className="flex gap-2 pt-1 border-t border-[#E2E8F0]">
        {isUpcoming && (
          <Link to={`/video-consult/${id}`}
            className="flex-1 flex items-center justify-center gap-2 py-2 rounded-btn text-white text-sm font-semibold hover:brightness-110 transition-all"
        ...
        {(status === 'PENDING' || status === 'CONFIRMED') && onCancel && (
          <button onClick={() => onCancel(id)}
            className="flex-1 flex items-center justify-center gap-2 py-2 rounded-btn border border-[#BA1A1A]/30 text-[#BA1A1A] text-sm font-semibold hover:bg-red-50 transition-colors">
